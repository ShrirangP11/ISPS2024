...
data <- read.csv("Chennai.csv",header = FALSE)
...
for(i in 1:73){
  year <- i + 1950
  count <- daily %>% filter(between(Date, as.Date(paste0(year,'-01-01')), as.Date(paste0(year,'-12-31')))) %>% filter(Value>115.6) %>% count()
  exc[i] <- count[1,1]
}
par(mfrow=c(1,1))
plot(exc,type='l')
# write.csv(exc, 'C:\\Users\\ShrirangP\\Documents\\GitHub\\ISPS2024\\chennai_exc.csv',row.names=F)
